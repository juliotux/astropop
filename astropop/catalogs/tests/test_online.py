...
def test_simbad_catalog_query_object():
    ...
    check.equal(obj[0]['MAIN_ID'], '* alf CMa')
    check.equal(obj[0]['RA'], '06 45 08.9172')
    check.equal(obj[0]['DEC'], '-16 42 58.017')
    check.equal(obj[0]['COO_BIBCODE'], '2007A&A...474..653V')
    npt.assert_approx_equal(obj[0]['FLUX_V'], -1.46)
    check.equal(obj[0]['FLUX_SYSTEM_V'], 'Vega')
    check.equal(obj[0]['FLUX_BIBCODE_V'], '2002yCat.2237....0D')
